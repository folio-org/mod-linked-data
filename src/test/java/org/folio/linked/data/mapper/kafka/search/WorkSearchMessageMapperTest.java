...
class WorkSearchMessageMapperTest {
  ...
  void toIndex_shouldReturnCorrectlyMappedIndex_fromWork() {
    ...
    assertContributor(linkedDataWork.getContributors().get(0), JURISDICTION_CREATOR_LABEL, JURISDICTION, true);
    assertContributor(linkedDataWork.getContributors().get(1), JURISDICTION_CONTRIBUTOR_LABEL, JURISDICTION, false);
    assertContributor(linkedDataWork.getContributors().get(2), creatorPerson.getLabel(), PERSON, true);
    assertContributor(linkedDataWork.getContributors().get(3), contributorOrg.getLabel(), ORGANIZATION, false);
    assertContributor(linkedDataWork.getContributors().get(4), wrongContributor.getLabel(), null, false);
    assertThat(linkedDataWork.getContributors()).hasSize(5);
